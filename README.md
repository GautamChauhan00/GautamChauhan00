# Hi, I'm Gautam Chauhan 👋

### BCA Graduate | MERN Stack Developer | WordPress Developer | Aspiring Data Analyst

I'm a BCA graduate with hands-on experience in building full-stack web applications using the MERN stack and developing professional websites using WordPress. I also work with SQL, Python, Pandas, Excel and Matplotlib for data analysis.

I enjoy building practical projects, learning new technologies, and solving real-world problems through code and data.

---

## 🛠️ Technical Skills

**Languages & Programming**
- JavaScript
- Python
- SQL

**Web Development**
- React.js
- Node.js
- Express.js
- MongoDB
- REST APIs

**WordPress Development**
- WordPress
- Theme Customization
- Plugin Management
- Responsive Website Development
- Website Maintenance

**Data Analysis**
- SQL / MySQL
- Python
- Pandas
- Matplotlib
- Microsoft Excel

**Tools**
- Git & GitHub
- MongoDB Compass
- Jupyter Notebook

---

## 🚀 Projects

### 🐾 Furry Souls
Full-stack donation platform designed to connect donors with animal shelters.

**Tech:** TypeScript

🔗 [View Project](https://github.com/GautamChauhan00/Furry-souls)

### ❄️ WellMake Engineer
MERN-based web application for a modular/cold room business with product listings, quotation management, cart & checkout, admin dashboard, order management and PDF invoice functionality.

**Tech:** MongoDB, Express.js, React.js, Node.js

🔗 [View Project](https://github.com/GautamChauhan00/Wellmake-MERN)

### 💼 Web Solutions

Professional responsive business and portfolio website developed using WordPress, Elementor and Astra, featuring dedicated service, about, contact and portfolio pages with contact form, SEO configuration and GitHub-integrated MERN projects.

*Tech:* WordPress, Elementor, Astra, WPForms, Rank Math SEO

🔗 [Project Repository](https://github.com/GautamChauhan00/web-solutions)
---

## 📚 Currently Learning

- Data Analytics
- SQL
- Python for Data Analysis
- Pandas
- Excel
- Power BI

---

## 🎓 Education

**Bachelor of Computer Applications (BCA)**

---

## 📫 Connect With Me

- 💼 [LinkedIn](https://www.linkedin.com/in/gautam-chauhan-910699340/)
- 🐙 [GitHub](https://github.com/GautamChauhan00)
